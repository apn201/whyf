...
a misrepresentation that can void an insurance policy. So the agent explains the
row and prices it, and you answer it.

### How it is built

Five stages, and the interface shows which one is running, what it did, how long
it took and which model it used, because from the outside you cannot otherwise
tell whether a model wrote the answer or a card did.

- Recogniser normalises the row (numbering, PDF hyphenation, translation noise),
  hashes it, checks DynamoDB. Repeat rows cost zero model calls.
- Librarian expands acronyms, then searches 112 knowledge cards two ways at
  once: a lexical index and cosine similarity over pre-embedded vectors. Returns
  a shortlist of 15. *Titan Embed v2.*
- Reader picks one concept, and *separately* judges whether that concept
  actually answers the row. *Claude Haiku 4.5.*
- Scribe assembles the verdict from the card. Deterministic; no model writes any
  of it.
- Understudy only runs when nothing covers the row, and writes one under heavy
...
Splitting "which concept" from "does it answer". Asked which concept a row is
...
`incident-reporting`. Right family, wrong answer, well formatted, which is the
worst output a tool like this can produce. Making those two separate judgements
took a probe set from 1/9 to 8/9. A confident *no* on the second one is the
useful answer: here is the nearest card, here is exactly what it does not cover.

Fixing retrieval instead of reaching for a bigger model. Ask it *"Do you have
...
answer perfectly well. That looks like comprehension failing, when the failure
is in retrieval. Fixed with an acronym table and a retrieval-only vocabulary on
33 cards, neither of which costs a model call.
...
gap, while non-security rows still decline, which is what makes the first number
mean anything.
